...
def get_vacancies():
    ...
    for vacancy, description in zip(vacancies, descriptions):
        ...
        if titleHref:
            ...
            description_items = [li.text.strip() for li in description.find_all('li')]
...
def get_detailed_description(link):
    ...
    required_skills = soup.find_all('div', class_='block-desc-col')[1].find_all('li')
    preferred_skills = soup.find_all('div', class_='block-desc-col')[2].find_all('li')
    ...
    for service in we_offer:
        service_name = service.find('div', class_='service-name block-el-title')
